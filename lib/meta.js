...
// Nuevos seguidores del mes. `follower_count` fue deprecado por Meta en varias cuentas
// (da error → "—"); si falla o viene 0, cae al fallback moderno `follows_and_unfollows`.
async function igNewFollowers(igId, since, until) {
  const fc = await igSum(igId, "follower_count", since, until);
  if (fc != null && fc !== 0) return fc;
  try {
    const j = await metaGet(`/${igId}/insights`, {
    ...
    if (tv) {
      ...
      for (const r of results) {
        const dim = (r.dimension_values || []).join(" ").toUpperCase();
        const val = Number(r.value) || 0;
        if (dim.includes("UNFOLLOW")) unfollows += val;
        else follows += val;
      }
      if (follows || unfollows) return follows - unfollows;
      if (typeof tv.value === "number") return tv.value;
    }
  } catch (_) {}
  return fc; // puede ser 0 o null
}
...
async function getInstagram(months) {
  ...
  for (const m of months) {
    ...
    const newFollowers = await igNewFollowers(igId, m.since, iu);
...
async function getMetaAds(months) {
  ...
  // La cuenta es compartida "MÁS CENTER / TIERRA CALMA". Excluimos las campañas de otras
  // marcas (por defecto Tierra Calma) para mostrar SOLO Más Center. Configurable por env.
  const EXCLUDE = (process.env.META_EXCLUDE_CAMPAIGNS || "tierra calma")
...
export async function getMetaDashboard() {
  ...
  // Facebook se omite a propósito: la página oficial casi no tiene audiencia
  // orgánica (≈89 seguidores) y el token no tiene insights sobre ella.
